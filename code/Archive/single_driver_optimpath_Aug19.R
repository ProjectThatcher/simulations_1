...
#     }
#     else{
#       optValues[k_ind] = 10000 #This number is made up and needs to change
#     }
#   }
#   return(optValues)
# }

getHashedCoverageByKeyForMCSimulation <- function(indices,optimPathHash,lambda_is, pMaj_is, majA_is,minA_is,age_is,m_is){
  lambda_i = lambda_is[indices]
  pMaj_i = pMaj_is[indices]
  majA_i = majA_is[indices]
  minA_i = minA_is[indices]
  age_i = age_is[indices]
  m_i = m_is[indices]
  
  key_t = paste(lambda_i, pMaj_i, majA_i,minA_i,age_i,m_i, sep = ",")
  # print(key_t)
  hash_val = optimPathHash[[key_t]]
  # print(hash_val)
  optValue = hash_val$optVal
...
getEVofMonitoringMonteCarlo <-function(optimPathHash,lambda_var,pMaj_var,m_i,m_tier_i,majA_init, minA_init,age_init,T_var = T){
  ...
  for(t_i in seq(T-1)){
    ...
    # getHashedCoverageByKeyForMCSimulation(optimPathHash,lambda_i=lambdas, pMaj_i=pMajs, majA_i=majA_state,minA_i=minA_state,age_i=age_is,m_i=m_is)
    ...
    validInds = which(majA_state < 4 & minA_state <7)
    # print(validInds)
    additions = rep(1000,numSimulations)
    coverageVals = sapply(validInds,getHashedCoverageByKeyForMCSimulation,optimPathHash=optimPathHash,lambda_is=lambdas, pMaj_is=pMajs, majA_is=majA_state,minA_is=minA_state,age_is=age_is,m_is=m_is)
    additions[validInds] = coverageVals
...
simulateOptCovPath <-function(){
  ...
  sim_path$age = rep(paste(lambda_i,pMaj_i,start_age_draw,","),T)
