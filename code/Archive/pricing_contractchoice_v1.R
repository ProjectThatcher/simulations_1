...
y_l = expand.grid(u_l = c(30, 100)
                , m = c(0,1))
y_l$keys = as.vector(paste(y_l$u_l, y_l$m, sep = ","))
...
y_c = expand.grid(u_c = c(0, 10)
                  , d_c = c(500, 2000)
                  , m = c(0,1))
y_c$keys = as.vector(paste(y_c$u_c, y_c$d_c, y_c$m, sep = ","))
...
X = expand.grid(hh_risk = c(1,2,3,4)
                #, age female marriage with ratable spouse
                #, priorins = c(0,1),
...
## P = P(X, A, y)
## P = Base rate by geo type * tier factor * coverage factor * hh_risk factor ( * credit * monitoring tier * car factor * priorins/fulcov)
require(hash)
...
Pricing <- function(geo_string, majA, minA, hh_risk, car_risk, u_l_000, u_c_000, d_c, m, m_tier, t){
  ...
  p_y_l = as.numeric(values(P_y_l_hash, keys = paste(u_l_000, m, sep=",")))
  p_y_c = as.numeric(values(P_y_c_hash, keys = paste(u_c_000, d_c, m, sep=",")))
  p_M = as.numeric(values(P_M_hash, keys = m_tier))
  if(t == 0){P = (p_l * p_y_l + p_c * p_y_c) * p_A * p_X}
  else{P = (p_l * p_y_l + p_c * p_y_c) * p_A * p_X * p_M}
  return(P)
}

## For someone with geo_type = C, A = (0,2), X = (2,2), y_l = (100000,1), y_c = (10000,2000,1)
Pricing("C", 0, 2, 2, 2, 100, 10, 2000, 1)
